  ...
  module Base
    module Record
      module ClassMethods
          ...
          types.each do |type|
            ...
            if klass
              if klass.column_names.include?('move_id')
                klass.find_each(:all, :conditions => conditions) do |record|
                  record.move_from
                end
              else
        ...
        def move_to(*types)
          conditions = types.pop
          types_with_move_id = types.select do |type|
            movable_class(type).column_names.include?('move_id')
          end
          unless types_with_move_id.empty?
            self.find_each(:all, :conditions => conditions) do |record|
              record.move_to(*types_with_move_id)
            end
          end
          (types - types_with_move_id).each do |type|
        ...
        def movable_class(type)
          eval(type.to_s.classify + self.table_name.classify) rescue nil
        end
      end
